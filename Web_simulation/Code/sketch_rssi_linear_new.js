...
var map_weight_loc = []; //Create a Map with Weight
var map_count = [];
...
function setup() {
    ...
    var aaaaa = [0, 2, 7, 6, 4, 8, 11, 11, 21, 44, 54, 62, 86, 93, 112, 114, 123, 148, 173, 173, 195, 210, 222, 227, 230, 254, 265, 299, 305, 318, 336, 362, 378, 405, 410, 420, 442, 438, 476, 518, 547, 546, 584, 619, 611, 656, 673, 707, 718, 728, 778, 798, 838, 872, 886, 907, 925, 950, 994, 1021, 1024, 1072, 1104, 1124, 1150, 1172, 1219, 1209, 1261, 1248, 1288, 1324, 1347, 1361, 1382, 1405, 1447, 1475, 1486, 1502, 1485, 1564, 1583, 1580, 1606, 1636, 1655, 1687, 1684, 1713, 1700, 1733, 1767, 1783, 1801, 1790, 1798, 1812, 1839, 1851];
    var bbb = "";
...
function predictPosition(large) {
    ...
    for (var i = 0; i < map_weight.length; i++) {
        for (var j = 0; j < map_weight[i].length; j++) {
            if (map_weight_loc[i][j] > 0 && map_count[i][j] > 0) {
                if (map_weight_loc[i][j] / map_count[i][j] > large[0]) {
                    large[0] = map_weight_loc[i][j] / map_count[i][j];
                }
            }
        }
    }
    ...
    for (var i = 0; i < map_weight.length; i++) {
        for (var j = 0; j < map_weight[i].length; j++) {
            if (map_weight_loc[i][j] > 0 && map_count[i][j] > 0) {
                if (map_weight_loc[i][j] / map_count[i][j] >= large[0] * 0.8) {
                ...
                if (startX + map_width > weight_i && weight_i > startX - map_width && startY > weight_j && weight_j > startY - map_width) { //be sure weight in local matrix
                    if (map_weight[weight_i][weight_j] >= 0) {
                        ...
                        if (dist <= cur_radius) {
                            ...
                            map_weight_loc[weight_i][weight_j] += dist / Math.pow((cur_radius), 2);
                            map_count[weight_i][weight_j] += 1;
                        } else {
                            map_weight[weight_i][weight_j] += (2*rssi - point_rssi)/rssi;
                            map_weight_loc[weight_i][weight_j] += (cur_radius * 2 - dist) / Math.pow((cur_radius), 2);
                            map_count[weight_i][weight_j] += 1;
                        }
