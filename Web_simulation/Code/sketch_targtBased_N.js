    ...
    while (targetX <= 650) {
        ...
        if (step_count < 100) {
            ...
            try {
                ...
                if (step_count < 10) {
                    descision = turnDecision(currentX, currentY, prepointX, prepointY);
                } else {
                    descision = tar_turnDecision(currentX, currentY, prepointX, prepointY, predictTarX, predictTarY);
                }
...
function tar_turnDecision(currX, currY, preX, preY, tarX, tarY) {
    ...
    if(turnCase!=7){
        ...
        //Previous Point
            stroke(52, 141, 231); //RGB&Opacity
            for (var i = new_tarX - 2; i < new_tarX + 3; i++) {
                for (var j = new_tarY - 2; j < new_tarY + 3; j++) {
                    point(i, j);
                }
            }
        if (new_tarY < currY) {
...
function triangulation(x, y, r){
    ...
    var m1 = (x[0] - x[1])/(y[1]-y[0]);
    var m2 = (x[1] - x[2])/(y[2]-y[1]);
    var m3 = (x[2] - x[0])/(y[0]-y[2]);
    var k1 = (pow(r[0],2) - pow(r[1],2) + pow(x[1],2) - pow(x[0],2) + pow(y[1],2) - pow(y[0],2)) / (2*(y[1]-y[0]));
    var k2 = (pow(r[1],2) - pow(r[2],2) + pow(x[2],2) - pow(x[1],2) + pow(y[2],2) - pow(y[1],2)) / (2*(y[2]-y[1]));
    var k3 = (pow(r[2],2) - pow(r[0],2) + pow(x[0],2) - pow(x[2],2) + pow(y[0],2) - pow(y[2],2)) / (2*(y[0]-y[2]));
    var a1 = 1+ pow(m1,2);
    var a2 = 1+ pow(m2,2);
    var a3 = 1+ pow(m3,2);
    var b1 = 2*(m1*k1-m1*y[1]-x[1]);
    var b2 = 2*(m2*k2-m2*y[2]-x[2]);
    var b3 = 2*(m3*k3-m1*y[0]-x[0]);
    var c1 = pow(x[1],2) + pow(y[1],2) + pow(k1,2) - 2*k1*y[1] - pow(r[1],2);
    var c2 = pow(x[2],2) + pow(y[2],2) + pow(k2,2) - 2*k2*y[2] - pow(r[2],2);
    var c3 = pow(x[0],2) + pow(y[0],2) + pow(k3,2) - 2*k3*y[0] - pow(r[0],2);
    var tx1 = [0,0];
    var ty1 = [0,0];
    var tx2 = [0,0];
    var ty2 = [0,0];
    ...
    //for r[2]
    tx1[0] = (-b1 + Math.sqrt(pow(b1,2) - 4*a1*c1))/2*a1;
    tx1[1] = (-b1 - Math.sqrt(pow(b1,2) - 4*a1*c1))/2*a1;
    ty1[0] = m1 * tx1[0] + k1;
    ty1[1] = re_calculate_weight_dist_more_100 * tx1[1] + k1;
    
    //for r[0]
    tx2[0] = (-b2 + Math.sqrt(pow(b2,2) - 4*a2*c2))/2*a2;
    tx2[1] = (-b2 - Math.sqrt(pow(b2,2) - 4*a2*c2))/2*a2;
    ty2[0] = m2 * tx2[0] + k2;
    ty2[1] = m2 * tx2[1] + k2;
    
    //for r[1]
    tx3[0] = (-b3 + Math.sqrt(pow(b3,2) - 4*a3*c3))/2*a3;
    tx3[1] = (-b3 - Math.sqrt(pow(b3,2) - 4*a3*c3))/2*a3;
    ty3[0] = m3 * tx3[0] + k3;
    ty3[1] = m3 * tx3[1] + k3;
    ...
    var tx = [0,0,0];
    var ty = [0,0,0];
    var w = [r[0]+r[1], r[1]+r[2], r[0]+r[2]];
    
    if( pow(tx1[0]-x[2],2) + pow(ty1[0]-y[2],2) <= pow(r[2],2) ){
        tx[0] = tx1[0];
    }
    else{
        tx[0] = tx1[1];
    ...
    if( pow(tx2[0]-x[0],2) + pow(ty2[0]-y[0],2) <= pow(r[0],2) ){
        tx[1] = tx2[0];
    }
    else{
        tx[1] = tx2[1];
    ...
    if( pow(tx3[0]-x[1],2) + pow(ty3[0]-y[1],2) <= pow(r[1],2) ){
        tx[2] = tx3[0];
    }
    else{
        tx[2] = tx3[1];
    ...
    var t = [0,0]; //Final Point
    var aw = (r[0]+r[1]+r[2]);
    
    t[0]  = ((tx[0]*w[0])/2*aw) + ((tx[1]*w[1])/2*aw) + ((tx[2]*w[2])/2*aw);
    t[1]  = ((ty[0]*w[0])/2*aw) + ((ty[1]*w[1])/2*aw) + ((ty[2]*w[2])/2*aw);
