...
function setup() {
    ...
    //radius = rssiToDist(rssi_filter[0]);

    radius = distance(currentX, currentY, targetX, targetY);

    rssi_filter = [];


    map_width = radius * 1; //Local matrix width
    //console.log(radius);
    ...
    */
    ...
    stroke(0, 7, 219); //RGB&Opacity
    //line(0,525, 1000, 525);
    


    stroke(0);
    line(0, currentY, 1000, currentY);
    ...
    for (var i = currentX - map_width; i <= currentX + map_width; i++) {
        for (var j = currentY; j >= currentY - map_width; j--) {
            var d = distance(i, j, currentX, currentY);
            if (d <= map_width) {
                stroke(0, 0, 0, 150*d/radius); //RGB&Opacity
                point(i,j)
            }
        }
    }
...
/*
    ...
    var ii = 0,
        jj = 0;
    for (var i = currentX - map_width; i <= currentX + map_width; i += 50) {
        for (var j = currentY; j >= currentY - map_width; j -= 50) {
            var d = distance(i, j, currentX, currentY);
            if (d <= map_width) {
                w[ii][jj] += (d / radius);
                /*
    ...
    //stroke(0, 7, 219); //RGB&Opacity
    //line(275, 675, 675, 275);


    var ii = 0,
        jj = 0;
    radius = distance(currentX, currentY, targetX, targetY);
    for (var i = startX - 200; i <= startX + 200; i += 50) {
        for (var j = startY; j >= startY - 200; j -= 50) {
            var d = distance(i, j, currentX, currentY);
            var sd = distance(i, j, startX, startY);
            
            if (d <= radius) {
                if (i+j <=  950 ) {
                    if (sd <= map_width) {
                        /*
                        stroke(202, 141, 31); //RGB&Opacity
                        for (var x = i - 2; x < i + 3; x++) {
                            for (var y = j - 2; y < j + 3; y++) {
                                point(x, y);
                            }
                        }
                        */
   /*                     
    ...
    for (var i = startX - 200; i <= startX + 200; i += 50) {
        for (var j = startY; j >= startY - 200; j -= 50) {
            var d = distance(i, j, startX, startY);
            
            for (var x = -25; x < 25; x++) {
                for (var y = -25; y < 25; y++) {
                    stroke(0, 0, 0, 200 * w[ii][jj] / 2);
                    point(i + x, j + y);
                }
            }
...
*/
    /*
    //Current Point
    stroke(102, 11, 231); //RGB&Opacity
    for (var x = startX - 2; x < startX + 3; x++) {
        for (var y = startY - 2; y < startY + 3; y++) {
            point(x, y);
        }
    }
*/
...
function mousePressed() {
            ...
            //var rssiFiltered = meidanFilter(rssi_filter);
            radius = rssiToDist(rssi_filter[0]);
            ...
            for (var x = startX - map_width; x <= startX + map_width; x++) {
                for (var y = startY - map_width; y < startY; y++) {
                    stroke(13, 151, 31, (map_weight[x][y]/map_count[x][y])); //RGB
                    point(x, y);
                }
            }
...
function addWeight(currX, currY, preX, preY, cur_radius, pre_radius) {
    ...
    if (cur_radius - pre_radius > 0) { //半徑變大，表示遠離
        for (i = currX - cur_radius; i <= currX + cur_radius; i++) { //-10+10用來額外預估
        ...
        for (i = currX - cur_radius; i <= currX + cur_radius; i++) { //-5+5用來額外預估
            for (j = currY; j >= currY - cur_radius; j--) {
                var weight_i = Math.round((i - currX) * r_matrix[0] + (j - currY) * r_matrix[1]) + currX;
                var weight_j = Math.round((i - currX) * r_matrix[2] + (j - currY) * r_matrix[3]) + currY;
                if (startX + map_width > weight_i && weight_i > startX - map_width && startY > weight_j && weight_j > startY - map_width) { //be sure weight in local matrix
...
function record_each100_point(d_error, move_d) {
    ...
    if (!isNaN(g_rssiFiltered) && !isNaN(m_rssiFiltered) && !isNaN(k_rssiFiltered) && !isNaN(rssi) && !isNaN(d_error) && !isNaN(move_d)) {
        ...
        g_all[step_count] += parseFloat(g_rssiFiltered.toFixed(7));
        m_all[step_count] += parseFloat(m_rssiFiltered.toFixed(7));
