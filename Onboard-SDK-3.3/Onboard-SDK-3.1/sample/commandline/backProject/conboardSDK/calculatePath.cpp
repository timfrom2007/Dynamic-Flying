...
void calculatePath::GpsPoint(DJI::onboardSDK::CoreAPI api, float nowLat, float nowLon, float nowAlt, float TargetLat, float TargetLon, float TargetAlt)
{
      ...
      float sampleRateNanoSec = 300000000L;
      float maxSpeed = 5.0;
      ...
      float RecordVx, RecordVy, RecordVz, Xoffset, Yoffset, Zoffset;
      while(n->nextAttitude != NULL)
      {
          //send()
          printf("ID = %d\n", n->nextAttitude->ID);
	  ...
	  pthread_t cid;
    	  int ret;
    	  ret = pthread_create(&cid, NULL, calibrationRecord, (void*)(&api));
    	  if(ret!=0){
               printf ("Create pthread error!\n");
               exit (1);
    	  } 
	  //RecordVx = flight->getVelocity().x;
          //RecordVy = flight->getVelocity().y;
          //RecordVz = flight->getVelocity().z;
          //Xoffset = RecordVx - n->nextAttitude->speedX;
          //Yoffset = RecordVy - n->nextAttitude->speedY;
          //Zoffset = RecordVz - n->nextAttitude->speedZ;
	  //cout << "(x, y, z) = (" << RecordVx << ", " << RecordVy << ", " << RecordVz << ")" << endl;
	  //cout << "(Xoffset, Yoffset, Zoffset) = (" << Xoffset << ", " << Yoffset << ", " << Zoffset << ")" << endl;
	  /*
	  if(Xoffset > (n->nextAttitude->speedX)/5){
	       n->nextAttitude->nextAttitude->speedX -= 2*Xoffset;
	  }
	  if(Yoffset > (n->nextAttitude->speedY)/5){
               n->nextAttitude->nextAttitude->speedY -= 2*Yoffset;
          }
	  if(Zoffset > (n->nextAttitude->speedZ)/5){
               n->nextAttitude->nextAttitude->speedZ -= 2*Zoffset;
          }*/
...
Attitude* calculatePath::GpsPoint( float nowLat, float nowLon, float nowAlt, float TargetLat, float TargetLon, float TargetAlt)
{
      float sampleRateSec = 0.8;
      float sampleRateNanoSec = 300000000L;
      float maxSpeed = 5.0;
