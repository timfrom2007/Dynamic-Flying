...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "http://140.122.184.219:4000/v_complete?value=true");
    ...
    t2->api = (void*)&api;
    t2->filename = datafilename;
    //pthread_t cD;
    //int ret;
    ret = pthread_create(&cD, NULL, compareData, (void*)t2);
    if(ret!=0){
         printf ("Create pthread error!\n");
         exit (1);
    }
    ...
    for(int i = 0; i < trip; i++){
        ...
        int Vy = -(velocity);
        int Vz = 0;
        float count = rangeLength / (velocity*0.7);
    ...
    int Vy = -(velocity);
    count = rangeLength / (velocity*0.7);
    for(int j = 0; j < count; j++){
        if(trip%2 != 0){
            Vy = velocity;
        }
...
void *compareData(void *ptr)
{
    ...
    fp.open(((threadData*)ptr)->filename, ios::app);
    if(!fp){
    ...
    while(CExit){
	 ...
	 fp << "-------------------------" << endl;
	 fp << "time = " << setprecision(10) <<  stime.str() << endl; 
